...
export default function FloatingHeader() {
  ...
  return (
    <div className={`w-full pt-4 px-2 transition-all flex justify-between bg-floating-header h-[100px] desktop:h-[150px] fixed z-20 ${show ? 'translate-y-0' : '-translate-y-full'}`}>
      <div className="w-full max-w-7xl mx-auto flex justify-between">
        <div>
          <Logo height={40} width={120} className={'mt-2 desktop:w-[200px] desktop:h-[80px]'} />
        </div>
        <div>
          <a href={downloadUrl} onClick={() => { EventBus.getInstance().fireEvent(ShowRegisterModalEvent); }} className={`${inter.variable} font-inter bg-gray-100 uppercase rounded-3xl flex items-center p-2 text-sm text-black desktop:mt-4`}>
            <span className='mr-2'>
