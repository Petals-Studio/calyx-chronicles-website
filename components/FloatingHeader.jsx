...
export default function FloatingHeader() {
  ...
  return (
    <div className={`w-full pt-4 px-2 transition-all flex justify-between bg-floating-header h-[100px] fixed z-20 ${show ? 'translate-y-0' : '-translate-y-full'}`}>
      <div className="w-full max-w-7xl mx-auto flex justify-between">
        <div>
          <Logo height={40} width={120} className={'mt-2'} />
        </div>
        <div>
          <button className={`${inter.variable} font-inter bg-gray-100 uppercase rounded-3xl flex items-center p-2 text-sm text-black`}>
            <span className=' mr-2'>
