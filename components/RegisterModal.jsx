...
export default function RegisterModal() {
  ...
  return (
    <div className="h-full w-full fixed top-0 left-0 z-20 grid place-items-center">
      <div
        className="overlay absolute h-full w-full bg-[#000] opacity-60"
        onClick={() => {
          setShow(false);
        }}
      ></div>
      <div className="h-screen w-screen max-w-full tablet:h-auto tablet:w-auto bg-[#1e1c1c] text-[#fff] flex flex-col tablet:grid tablet:place-items-center py-9 relative px-4 tablet:px-12">
        <div>
          <button
            onClick={() => {
              setShow(false);
            }}
            className="tablet:hidden px-2 py-1"
          >
            <span>
              <svg
                width="27"
        ...
        <h4
          className={`${inter.variable} font-inter text-muted font-bold uppercase my-2 tracking-wide`}
        >
          Coming soon
        </h4>

        <h2
          className={`${albertusNova.variable} font-albertus-nova text-[rgba(255,255,255,0.65)] tablet:text-2xl uppercase tracking-wide mt-6`}
        >
        ...
        <form
          ...
          className="flex flex-col w-[360px] mt-6 tablet:mt-3"
        >
          <label
            htmlFor="new-email"
            className={`${inter.variable} font-inter bold text-base mb-2 text-muted`}
          >
            EMAIL
          </label>
          ...
          <div className="flex tablet:justify-center mt-4">
            <button
              className={`${inter.variable} font-inter bg-[#d9d9d9] text-[#000] uppercase px-6 rounded-[5px] cursor-pointer`}
              type="submit"
            >
              SUBSCRIBE
            </button>
